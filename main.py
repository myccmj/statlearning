...
def load_data(args):
    ...
    if args.dataset == 'gisette':
        x_data = np.array(sio.loadmat('./data/gisette/gisette_train_scale.mat')['A'].todense()).astype(np.float32)
        y_data = sio.loadmat('./data/gisette/gisette_labels.mat')['b'].astype(np.float32)
        ind_0 = np.reshape(y_data==-1, -1)
        ind_1 = np.reshape(y_data==1, -1)
...
if __name__ == "__main__":
    ...
    file_name = args.dataset + args.classifier + args.method
    train_op.save_results(filename=file_name)
